    ...
    for (const i in features) {
      if (i == name) {
        if (Array.isArray(obj[i])) {
          obj[i].length
            ? obj[i].map((val: any, idx: any) =>
                val == check
                  ? obj[i].splice(idx, 1)
                  : obj[i].includes(check)
                  ? obj[i].splice(obj[i].indexOf(check), 1)
                  : obj[i].push(check)
              )
            : obj[i].push(check);
          console.log(obj[i]);
  ...
  const CARCLASS = [
    {
      id: 4,
      name: "Yo’l-yo’lakay",
      price: price?.economy ?? 0,
      Images: ByWayCarImage,
    },
    {
      id: 1,
      name: "Standart",
      price: price?.standard ?? 0,
      Images: StandarCarImgIcon,
    },
    {
      id: 2,
      name: "Komfort",
      price: price?.comfort ?? 0,
      Images: ConfortCarImgIcon,
    },
    {
      id: 3,
      name: "Business",
      price: price?.business ?? 0,
      Images: BusnissCarImgIcon,
    },
  ];
    ...
    <>
      ...
      <div className={cls.flex}>
        ...
        <div className={`${cls.parent} grid grid-cols-2 gap-6`}>
          <div
            onClick={() => handleCheck("driver_gender", "m")}
            className={`cursor-pointer flex items-center gap-2 pl-[14px] font-medium border border-[var(--lineGray)] rounded-[10px] h-[48px]`}
          >
            <div
              className={`w-[18px] h-[18px] rounded-[4px] border-2 ${
                features.driver_gender.includes("m")
          ...
          <div
            onClick={() => handleCheck("driver_gender", "f")}
            className={`cursor-pointer flex items-center gap-2 pl-[14px] font-medium border border-[var(--lineGray)] rounded-[10px] h-[48px]`}
          >
            <div
              className={`w-[18px] h-[18px] rounded-[4px] border-2 ${
                features.driver_gender.includes("f")
                  ? "border-[var(--main500)] bg-[var(--main500)]"
                  : "border-[var(--lineGray)]"
              }`}
            >
        ...
        <div className={cls.parent}>
          <div className="flex items-center gap-4">
            {CARCLASS?.map(({ id, name, price, Images }) => (
              <div
                onClick={() => handleCheck("car_class_id", id)}
                className={`p-3 ${
                  features.car_class_id == id ? "bg-[var(--lineGray)]" : ""
                } hover:bg-[var(--lineGray)] rounded-xl cursor-pointer`}
              >
                <Images />
                <div className="mt-4">
                  <p className="text-xs text-[var(--gray)] font-medium">
                    {name}
                  </p>
                  <p className="text-base font-semibold text-[var(--black)]">
                    {price} so'm
                  </p>
                </div>
              </div>
