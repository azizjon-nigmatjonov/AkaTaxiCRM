...
const PassengerProfile = () => {
  ...
  const SelecTList = useMemo(() => {
    ...
    return regions.data.map((item: any) => {
      return {
        ...item,
        label: item.name?.uz,
        value: item.id,
  ...
  useEffect(() => {
    if (query?.passenger === "delete") {
      setAlert("Haqiqatdan ham o’chirishni istaysizmi?");
    }
    if (query?.passenger === "update") {
  ...
  const alertMessage = (e: string) => {
    if (e == "delete") {
      passengerService.deleteElement(id).then(() => {
        ...
        navigateTo("/passengers/passenger-list");
      });
      ...
      passengerService.updateElement(id, data).then(() => {
        dispatch(
          websiteActions.setAlertData({
            ...
            <div className="w-full">
              <div className="w-full  grid grid-cols-3  gap-5">
                ...
                <HFDatePicker
                  ...
                  defaultValue={passenger?.birthday}
                />
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex items-centet gap-2">
            <button
              ...
              onClick={() => navigateTo("/passengers/passenger-list")}
            >
              Bekor qilish
            </button>
            <button
              className={`${
                checkPermission("edit") ? "" : "disabled"
              } custom-btn`}
              type={checkPermission("edit") ? "submit" : "button"}
            >
              Saqlash
            </button>
          </div>
