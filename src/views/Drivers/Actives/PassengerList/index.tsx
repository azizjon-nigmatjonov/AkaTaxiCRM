...
const PassengerList = ({ data }: { data: any }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        {pasengerList?.length ? pasengerList?.map((val: any) => (
          <div className="flex items-center justify-between py-4 border-b ">
            <div className="flex items-center gap-3">
              <Avatar alt="" src={val.photo} />
              <div>
                <p className="text-[var(--black)] font-semibold text-sm">
                  {val.full_name}
                </p>
        ...
        )) : <img className="w-[180px] mx-auto mt-5" src="/images/no-data.png" alt="empty"/>}
      </div>
    </CModal>
  );
};
