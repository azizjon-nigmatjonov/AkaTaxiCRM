...
const fileService = {
  ...
  getTrips: (data?: any) => request.get(`/trips?${data.page ? `page=${data.page}` : ''}${data.perPage ? `&perPage=${data.perPage}` : ''}${data.from_location_id? `&from_location_ids=${data.from_location_id}`:''}${data.to_location_id ? `&to_location_ids=${data.to_location_id}`:''}`)
};
